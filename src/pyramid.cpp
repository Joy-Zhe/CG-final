...
Pyramid::Pyramid(std::string texture_path) {
...
void Pyramid::draw(const glm::mat4& projection, const glm::mat4& view, std::shared_ptr<DirectionalLight> _light, glm::vec3 position, std::shared_ptr<AmbientLight> _ambientLight, std::shared_ptr<SpotLight> _spotLight, std::shared_ptr<Material> _material) {
...
{
    // 顶点坐标
    glm::vec3 topVertex = glm::vec3(0.0f, 0.0f, height / 2.0f);
    glm::vec3 bottomCenterVertex = glm::vec3(0.0f, 0.0f, -height / 2.0f);
    float angleIncrement = 2.0f * glm::pi<float>() / 5.0f;
    float currentAngle = 0.0f;

    // 生成顶面顶点和底面顶点
    vertices[0].position = topVertex; // 顶点
    vertices[0].color = glm::vec3(0.0f, 0.0f, 0.0f);
    vertices[0].texCoord = glm::vec2(0.5f, 0.5f); // 纹理坐标

    vertices[1].position = bottomCenterVertex; // 底点
    ...
    for (int i = 0; i < 5; i++)
    {
        glm::vec3 vertexPos = glm::vec3(radius * std::cos(currentAngle), radius * std::sin(currentAngle), -height / 2.0f);
        vertices[2 + i].position = vertexPos;
        vertices[2 + i].color = glm::vec3(0.0f, 0.0f, 0.0f);
        vertices[2 + i].texCoord = glm::vec2((std::cos(currentAngle) + 1.0f) / 2.0f, (std::sin(currentAngle) + 1.0f) / 2.0f);
